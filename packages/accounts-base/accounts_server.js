...
Ap._attemptLogin = function (
  ...
  var user;
  if (result.userId)
    user = this.users.findSingle(result.userId);
...
Ap._reportLoginFailure = function (
  ...
  if (result.userId) {
    attempt.user = this.users.findSingle(result.userId);
  }
...
Ap._expireTokens = function (oldestValidDate, userId) {
  ...
  this.users.expireTokens(oldestValidDate, userId);
  /*
...
//  var userId;
  ...
  return this.users.insertUser(fullUser);
...
Ap.updateOrCreateUserFromExternalService = function (
  ...
  var user = this.users.findByService(selector);
...
Ap._deleteSavedTokensForUser = function (userId, tokensToDelete) {
  ...
  this.users.deleteSavedTokens(userId, tokensToDelete);
/*
  if (tokensToDelete) {
    this.users.update(userId, {
      $unset: {
...
Ap._deleteSavedTokensForAllUsersOnStartup = function () {
  ...
  Meteor.startup(function () {
    ...
    self.users.deleteSavedTokensforAllUsers();
    /*
    self.users.find({
      "services.resume.haveLoginTokensToDelete": true
    }, {
